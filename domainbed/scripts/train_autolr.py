...
from domainbed.lib.fast_data_loader import InfiniteDataLoader, FastDataLoader, InfiniteDataLoaderWithoutReplacement, FastDataLoader_no_shuffle
...
if __name__ == "__main__":
    ...
    out_splits = []
    train_domain_labels = []
    for env_i, (env, diff_env) in enumerate(dataset):
        ...
        out_diff, in_diff = misc.split_dataset(
            diff_env, int(len(diff_env) * args.holdout_fraction), misc.seed_hash(args.trial_seed, env_i)
        )
        if env_i in args.test_envs:
            ...
            uda_diff, in_diff = misc.split_dataset(
                in_diff, int(len(in_diff) * args.uda_holdout_fraction), misc.seed_hash(args.trial_seed, env_i)
            )
    ...
    print(len(train_data), len(test_data), len(train_data_diff), len(test_data_diff))

     # print image sizes
    img, _ = train_data[0][0]
    ...
    if "GUIDE" in args.algorithm:
        train_loader = FastDataLoader_no_shuffle(
            dataset=train_data_diff,
            batch_size=8,
            num_workers=0,
        )
        test_loader = FastDataLoader_no_shuffle(
            dataset=test_data_diff,
            batch_size=8,
            num_workers=0,
        )
    ...
    train_loaders = [
        InfiniteDataLoaderWithoutReplacement(
    ...
    eval_loaders_iid = [
        ...
        for i, idx in enumerate(test_idx_split) if i not in args.test_envs
    ]
    ...
    eval_weights = [None for _ in range(len(eval_loaders))]
