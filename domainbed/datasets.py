...
class ImageFolderWithTransforms(ImageFolder):
    def __init__(self, root, primary_transform, secondary_transform):
        super().__init__(root, transform=primary_transform)
    ...
    def __getitem__(self, index):
        ...
        primary_image = self.transform(sample) if self.transform else sample
        secondary_image = (
            self.secondary_transform(sample) if self.secondary_transform else sample
        )

        return primary_image, secondary_image, target


class ImageFolderWithTransforms_PreComputeFeat(ImageFolder):
    def __init__(self, root, primary_transform, secondary_transform, hparams):
        super().__init__(root, transform=primary_transform)
        self.secondary_transform = secondary_transform
        self.hparams = hparams
        self.dataset_root = "domainbed/data"

    def __getitem__(self, index):
        # Load the original sample
        path, target = self.samples[index]
        # Dynamically calculate the corresponding .npy path
        dataset_root = self.dataset_root.rstrip("/")  # Ensure no trailing slash

        # Compute relative path
        relative_path = os.path.relpath(
            path, start=dataset_root
        )  # e.g., PACS/art_painting/dog/pic_248.jpg

        # Dynamically ensure the relative path matches the feature save directory structure
        feat_save_root = os.path.basename(
            self.hparams["feat_save_name"]
        )  # Extract the last component of feat_save_name (e.g., "PACS")
        if relative_path.startswith(feat_save_root + "/"):
            relative_path = os.path.relpath(
                relative_path, start=feat_save_root
            )  # Strip the feat_save_root prefix

        # Combine the base feature save directory with the modified relative path
        feature_path = os.path.join(
            self.hparams["feat_save_name"], os.path.splitext(relative_path)[0] + ".npy"
        )
        sample = self.loader(path)

        # Apply both transforms
        primary_image = self.transform(sample) if self.transform else sample
        # secondary_image = self.secondary_transform(sample) if self.secondary_transform else sample

        if os.path.exists(feature_path):
            # load npy file from feature path
            feature = torch.tensor(np.load(feature_path))

            return primary_image, "None", target, path, feature_path, feature
        else:
            secondary_image = (
                self.secondary_transform(sample) if self.secondary_transform else sample
            )
...
class MultipleEnvironmentImageFolder_PreComputeFeat(MultipleDomainDataset):
    def __init__(self, root, test_envs, augment, hparams):
        ...
        if "feature_model" in hparams:

            if hparams["feature_model"] == "mae":
            ...
            elif hparams["feature_model"] in ["dit", "diffusion"]:
                diffusion_transform = transforms.Compose(
                    [
            ...
            elif hparams["feature_model"] == "clip":
                diffusion_transform = transforms.Compose(
                    [
            ...
            elif hparams["feature_model"] == "dino":
                diffusion_transform = transforms.Compose(
                    [
                        transforms.Resize(
                            518, interpolation=transforms.InterpolationMode.BICUBIC
                        ),  # Resize shortest edge to 518
                        transforms.CenterCrop((518, 518)),  # Center crop to 518x518
                        transforms.ConvertImageDtype(
                            torch.float32
                        ),  # Convert to float32 (rescale factor is 1/255)
                        transforms.Normalize(
                            mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
                        ),  # Normalize with ImageNet stats
                    ]
                )
            elif hparams["feature_model"] == "flux":
                diffusion_transform = transforms.Compose(
                    [
                ...
                diffusion_transform = transform
        else:
            diffusion_transform = transform

        self.datasets = []
        for i, environment in enumerate(environments):
            # Determine which transform to use for the first image
            if augment and (i not in test_envs):
                primary_transform = augment_transform
            else:
                primary_transform = transform

            path = os.path.join(root, environment)
            combined_dataset = ImageFolderWithTransforms_PreComputeFeat(
                path,
                primary_transform=primary_transform,
                secondary_transform=diffusion_transform,
                hparams=hparams,
            )
...
class MultipleEnvironmentImageFolder(MultipleDomainDataset):
    def __init__(self, root, test_envs, augment, hparams):
        ...
        for i, environment in enumerate(environments):
            ...
            path = os.path.join(root, environment)
            combined_dataset = ImageFolderWithTransforms(
                path,
                primary_transform=primary_transform,
                secondary_transform=diffusion_transform,
            )
...
class PACS_precomputefeat(MultipleEnvironmentImageFolder_PreComputeFeat):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["A", "C", "P", "S"]
...
class VLCS_precomputefeat(MultipleEnvironmentImageFolder_PreComputeFeat):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["C", "L", "S", "V"]
...
class TerraIncognita_precomputefeat(MultipleEnvironmentImageFolder_PreComputeFeat):
    CHECKPOINT_FREQ = 300
...
class PACSWithDomainLabels(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["A", "C", "P", "S"]
    ...
    def __init__(self, root, test_envs, hparams):
        ...
        # Precompute the domain labels for all samples
        self.domain_labels = self.precompute_domain_labels()

    def precompute_domain_labels(self):
        domain_labels = []
        for i, dataset in enumerate(self.datasets):
            domain_labels.extend([self.ENVIRONMENTS[i]] * len(dataset))
        return domain_labels

    def __getitem__(self, index):
        # Get the image and class label using the original dataset implementation
        img, class_label = super().__getitem__(index)

        # Fetch the precomputed domain label
...
class DomainNet_precomputefeat(MultipleEnvironmentImageFolder_PreComputeFeat):
    CHECKPOINT_FREQ = 1000
    ENVIRONMENTS = ["clip", "info", "paint", "quick", "real", "sketch"]
...
class OfficeHome_precomputefeat(MultipleEnvironmentImageFolder_PreComputeFeat):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["A", "C", "P", "R"]
...
class SpawriousBenchmark(MultipleDomainDataset):
    ...
    def __init__(
        ...
        train_datasets, test_datasets = self._prepare_data_lists(
            train_combinations, test_combinations, root_dir, augment
        )
