...
class GUIDE(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(GUIDE, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.cluster_featurizer = networks.ClusterFeaturizer(input_shape, self.hparams)

        # Determine feature dimensions
        feature_dim = self.featurizer.n_outputs
        cluster_dim = self.hparams["cluster_dim"]

        # Determine classifier input dimension
        classifier_input_dim = feature_dim + (cluster_dim)

        # Classifier
        self.classifier = networks.Classifier(
            classifier_input_dim, num_classes, self.hparams["nonlinear_classifier"]
        )

        # Gather model parameters
        self.params = list(self.featurizer.parameters()) + list(
    ...
    def predict(self, x, clust):
        ...
        clust_features = clust
        combined_features = torch.cat([image_features, clust_features], dim=1)

        return self.classifier(combined_features)

    def update(self, minibatches, device=None):
        # Aggregate minibatches
        all_x = torch.cat([x for x, clust, y in minibatches]).to(device)
        all_y = torch.cat([y for x, clust, y in minibatches]).to(device)
        all_clust = torch.cat([clust for x, clust, y in minibatches]).to(device)
...
class GUIDEPlusPlus(Algorithm, ErmPlusPlusMovingAvg):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(GUIDEPlusPlus, self).__init__(
        ...
        self.network = GUIDENetwork(self.featurizer, self.classifier)
        if self.hparams["lars"]:
            self.optimizer = LARS(
                self.network.parameters(),
                lr=self.hparams["lr"],
        ...
        linear_parameters = []
        for n, p in self.network.classifier.named_parameters():
            linear_parameters.append(p)
    ...
    def update(self, minibatches, device=None):
        """
        Update logic incorporating warm-up and clustering losses.
        """
        ...
        # Compute clustering-aware predictions
        preds = self.network(all_x, all_clust)
        ...
        if not self.hparams["freeze_bn"]:
            self.network_sma.train()
            self.network_sma(all_x, all_clust)
    ...
    def predict(self, x, clust):
        """
        Predict using the SMA network with clustering features.
        """
        self.network_sma.eval()
        with torch.no_grad():
            return self.network_sma(x, clust)
