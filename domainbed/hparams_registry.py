...
def _hparams(algorithm, dataset, random_seed):
    ...
    elif algorithm in ["GUIDE", "GUIDEPlusPlus"]:
        _hparam("num_clusters", 5, lambda r: int(r.choice([1, 3, 5])))
        _hparam("pca_dim", 512, lambda r: 512)
        _hparam("offset", 0, lambda r: 0)
        _hparam("clust_epoch", 0, lambda r: 0)
        _hparam("feature_model", "dit", lambda r: "dit")
        _hparam(
            "model_name",
            "facebook/DiT-XL-2-512",
            lambda r: "facebook/DiT-XL-2-512",
        )
        _hparam("diffusion_step_type", "onestep", lambda r: "onestep")
        _hparam("timestep", 50, lambda r: 50)
        _hparam("cluster_once", False, lambda r: False)
        _hparam("cosine_sched", "None", lambda r: "None")
        _hparam("align_reproj", False, lambda r: False)
        _hparam("clust_every_epoch", False, lambda r: False)
        _hparam("cluster_space", "org", lambda r: "org")
        _hparam("random_centroids", False, lambda r: False)
        _hparam("const_num_clusters", False, lambda r: False)
        _hparam("apply_layernorm", False, lambda r: False)

        if algorithm == "GUIDEPlusPlus":
            _hparam("linear_lr", 5e-5, lambda r: 10 ** r.uniform(-5, -3.5))
